...
import {
  ...
  Raycaster,
  Plane, MeshPhysicalMaterialParameters, WebGLProgramParametersWithUniforms,
} from "three";
...
class X {
  ...
  updateWorldSize() {
    if (this.camera.isPerspectiveCamera) {
      ...
      const cam = this.camera as unknown as THREE.OrthographicCamera;
  ...
  clear() {
    this.scene.traverse((obj) => {
      // Vérifie que l'objet est un Mesh
      if (obj instanceof THREE.Mesh) {
        const material = obj.material;

        // Vérifie si le matériau existe et est un objet
        if (material && typeof material === "object") {
          // Parcourt les propriétés du matériau
          Object.keys(material).forEach((key) => {
            const matProp = material[key];

            // Si la propriété du matériau a une méthode dispose, on l'appelle
            if (matProp && typeof matProp === "object" && typeof matProp.dispose === "function") {
              matProp.dispose();
            }
          });

          // Dispose le matériau et la géométrie du Mesh
          material.dispose();
...
function createBallpit(
  ...
  function initialize(cfg: { followCursor: boolean } | undefined) {
    if (spheres) {
      threeInstance.clear();
